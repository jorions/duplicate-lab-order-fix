...
module.exports = async (isDryRun, trx) => {
  ...
  for (const { labReorderRequestId, updatedNewLabOrderId } of updatedNewLabOrderIds) {
    ...
    if (updatedNewLabOrderId && updatedNewLabOrderId === lrr.originalLabOrderId)
      throw new Error(
        `The Lab Reorder Request ${labReorderRequestId} was going to have its NewLabOrderId ` +
          `updated to ${updatedNewLabOrderId} but that is the same as its current OriginalLabOrderId`,
      );
    // eslint-disable-next-line no-await-in-loop
