...
const getUpdatedNewLabOrderIds = async trx => {
  ...
  Object.entries(timelines).forEach(([patientRegistrationId, timeline]) => {
    ...
    timeline.forEach((evt, idx) => {
      ...
      const futureLabOrdersWithMatchingLabKitReference =
      ...
      const newestLabOrderWithMatchingLabKitReference =
        futureLabOrdersWithMatchingLabKitReference[
          futureLabOrdersWithMatchingLabKitReference.length - 1
        ];

      updatedNewLabOrderIds[labReorderRequestId] = {
        updatedNewLabOrderId: newestLabOrderWithMatchingLabKitReference
          ? newestLabOrderWithMatchingLabKitReference.labOrderId
          : newLabOrderId,
...
module.exports = async (isDryRun, trx) => {
  ...
  for (const { labReorderRequestId, ...data } of updatesToStore) {
    ...
    if (
      ...
      throw new Error(
        `The Lab Reorder Request ${labReorderRequestId} was going to have its OriginalLabOrderId ` +
          `updated to ${data.originalLabOrderId} but that is the same as its current NewLabOrderId`,
      );
    if (
      data.newLabOrderId &&
      ...
      throw new Error(
        `The Lab Reorder Request ${labReorderRequestId} was going to have its NewLabOrderId ` +
          `updated to ${data.newLabOrderId} but that is the same as its current OriginalLabOrderId`,
      );
    // eslint-disable-next-line no-await-in-loop
