...
const getUpdatedNewLabOrderIds = async trx => {
  ...
  const lrrToLog = getFlag('lrr', true);
  ...
  Object.entries(timelines).forEach(([patientRegistrationId, timeline]) => {
    timeline.forEach((evt, idx) => {
      ...
      if (labReorderRequestId === lrrToLog) {
        console.log('=====');
        console.log(`Logging Lab Reorder Request ${lrrToLog}'s timeline:`);
      ...
      // Use the LabOrderId of the newest Lab Order that has the same LabKitReference
      // as the Lab Order created by the Reorder Request. Default to the current LabOrderId
      // if there are none. If this is the only update for the Lab Reorder Request,
      // it will not change anything in the DB so it will not get persisted, and
      // will get logged as a skipped Reorder Request.
      const futureLabOrdersWithMatchingLabOrderId = futureEvents.filter(
        futureEvt => !futureEvt.isReorder && futureEvt.labKitReference === evt.labKitReference,
      );

      updatedNewLabOrderIds[labReorderRequestId] = {
        updatedNewLabOrderId:
          futureLabOrdersWithMatchingLabOrderId[futureLabOrdersWithMatchingLabOrderId - 1] ||
          newLabOrderId,
