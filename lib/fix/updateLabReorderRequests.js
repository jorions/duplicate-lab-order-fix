...
const parseLabOrderUpdates = (updatedOriginalLabOrderIds, updatedNewLabOrderIds) => {
  ...
  Object.entries(allData).forEach(
    ([
      ...
      const updateToStore = { labReorderRequestId };
...
module.exports = async (isDryRun, trx) => {
  ...
  for (const { labReorderRequestId, ...data } of updatesToStore) {
    ...
    if (data.originalLabOrderId && data.originalLabOrderId == lrr.newLabOrderId) // eslint-disable-line
      throw new Error(
    ...
    if (data.newLabOrderId && data.newLabOrderId == lrr.originalLabOrderId) // eslint-disable-line
      throw new Error(
